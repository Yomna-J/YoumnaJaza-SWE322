<?php
            ...
            while ($result_row = mysqli_fetch_row($result)) { # Loop over all classes
                if($result_row[4] < $result_row[5]){ # if it doesn't exceed max
                    if(date('Y-m-d') <= $result_row[3]){ # checks if the class date has passed
                        ...
                        $num_all_trainees += array($result_row[0], $result_row[4]);// array of all classes id , number of trainees 
                    }           
                }
    ...
    if(empty($classes)) 
    {
   echo("You didn't select any classes.");
    } 
   else 
  {
   ...
   for($i=0; $i < $num_classes; $i++)
    {
    $new_num_trainees = $num_all_trainees[$classes[$i]];
